...
updated-by: Codex
change-summary: 战报显示Live当前/预计组合IMR与66.6%上限；Demo继续显示实时max-size容量。
role: 推送 format=3 模板（scripts/push_pipeline.py 纯脚本唯一路径 -> OKX_QQ_TARGET 业务路由 + reports/ 归档）
...
# 推送模板 — format=3 战报 -> OKX_QQ_TARGET 业务路由
...
> 战报属于业务消息：调用 `qq_push.py` 时不得带 `--alert`，目标仅取 `OKX_QQ_TARGET`；模板和调用方禁止写真实 target/secret。P0、凭证或运行异常告警必须带 `--alert`，目标仅取 `OKX_QQ_ALERT_TARGET`。不同用途用显式 `dedupe-key` 区分。零模型名。
...
🟡 模拟盘：资金 $X | snapshot availBal $X(展示·非开仓容量) | 累计收益(交易PnL·未扣费) X USDT | M仓

💼 持仓详情（每仓一行；空仓写"空仓"）

🛡 风控  Live组合保证金 当前 X% | 有开单则预计 Y% / 66.6% | Demo开仓 X张/实时容量 Y张(max-size) | 杠杆 Xx/10x | 同侧 X%(观察) | 持仓 live N / demo M(数量仅观察) | PASS
...
| 模拟盘资产 | `🟡 模拟盘` | account_snapshots(profile=demo) 权威回读资产/绩效展示（**禁用 live totalEq 填 demo 槽；不得冒充开仓容量**） |
...
| 风控 | `🛡 风控` | profile 风险留痕：Live 显示当前组合 `account.imr/totalEq`，有 OPEN/ADD 时追加预计成交后比例，并对照 66.6%；严禁用 `mgnRatio`、gross、net 替代。Demo 显示实际开仓张数与本次 OKX Demo max-size，不套 Live 组合 IMR 或人工百分比公式；杠杆10x、SL距30%和SL必挂为共享安全边界 |
...
- Demo 资产行的 `totalEq/availBal` 只用于资产与绩效展示；开仓容量必须来自该笔 OPEN 回执的 `capacity.source=account.max-size` / `capacity.max_size`。无 Demo OPEN 时写“本轮无开/加仓”，不得拿快照 `availBal` 补容量。
...
#    业务战报不得带 --alert，目标仅取 OKX_QQ_TARGET；禁 channels PUT 旧伪代码、禁写真实目标。
```
...
**push_archive rc=2**：已存证但内容非渲染模板（缺『第N轮』header / 缺『📊』段 / **< 300 字符**）-> **禁外发当前内容**，回 render -> validate 重走。归档异常属于运行告警，必须经 `qq_push.py --alert` 推送至 `OKX_QQ_ALERT_TARGET`，并使用独立告警用途键。
...
| render 失败 | repair_queue + 经 `qq_push.py --alert` 告警（P2，目标仅取 `OKX_QQ_ALERT_TARGET`；不阻塞） |
...
| 业务/告警分流 | 业务战报不带 `--alert` 且仅取 `OKX_QQ_TARGET`；告警必须带 `--alert` 且仅取 `OKX_QQ_ALERT_TARGET`；禁写真实 target/secret |
