...
def _findings(out: str, cap: int = 8) -> str:
    lines = [ln.strip() for ln in out.splitlines()
             if any(w in ln for w in _MARK_WORDS)]
...
def main() -> int:
    ...
    for profile in ("demo", "live"):
        ...
        if profile == "live":
            ...
            alerts.append(
                f"[P1] live 账实差异 rc={rc}({'EXACT可修' if rc == 1 else 'FUZZY含糊'}): "
                f"{entry['findings']}｜人工: reconcile_exchange_closes.py --profile live"
                f"{' --apply（仅 GHOST-EXACT）' if rc == 1 else '（含糊面需逐笔核实）'}")
            continue
        # demo 分级
        if rc == 3:
        ...
        if rc_apply == 0 and rc_check == 0:
            ...
            log(f"demo GHOST-EXACT 已自动补账并复检归零")
        else:
            alerts.append(
    ...
    if alerts and not args.dry_run:
        ...
        text = (f"⚠️ 日频对账告警 [{f'{now_cst():%Y-%m-%d %H:%M}'}] (统一QQ告警)\n"
        ...
        cmd = [PWSH, "-NoProfile", "-File", WRAP, QQ_PUSH,
               "--content-file", str(f), "--dedupe-key", f"reconcile:{stamp}"]
        try:
            p = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8",
                               errors="replace", timeout=60,
                               creationflags=_CREATE_NO_WINDOW)
            report["alert_sent"] = {"rc": p.returncode}
            log(f"告警已推 rc={p.returncode}: {len(alerts)} 项")
        except Exception as e:
            report["alert_sent"] = {"error": str(e)}
    elif alerts:
        log(f"[dry-run] 将告警 {len(alerts)} 项（未推）")
