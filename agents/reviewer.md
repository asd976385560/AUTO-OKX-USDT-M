...
> 推送命令：把报告渲染到 UTF-8 文件，再 `pwsh -NoProfile -File <PROJECT_ROOT>/scripts/run_okx_python.ps1 <PROJECT_ROOT>/scripts/qq_push.py --content-file <该文件> --dedupe-key reviewer:<YYYY-MM-DD>:<用途>`（用途=daily/补正/告警；同日同用途重跑幂等。2026-07-16 起去重身份由调用方显式声明，不传键=按内容 hash 去重）
> ⚠️ **禁内联 pwsh 中文 here-string 拼 push 文本**（GBK 控制台必坏码、推送失败）：渲染 push 文本**必经 Python 写文件**——临时 `.py` 里 `open(path,'w',encoding='utf-8').write(text)`（经 `run_okx_python.ps1` 跑），或 `echo '<JSON>' | run_okx_python.ps1 <render脚本> --out <file>`；**绝不**用 pwsh 内联 here-string 拼中文（红线 #13：中文不走 pwsh 内联 / `python -c`）。然后才 `qq_push.py --content-file`。
...
> **唯一职责**：日/周/月复盘 → demo/live 绩效追踪报告（盈利验证）→ 经 `daily_report_writer` 落 `account.db` + `reports/daily-reports/` → 经 `qq_push.py` 推统一默认 target。
>
...
| 采集监控（纯脚本 `scripts/collection_monitor.py`，on-demand） | within-day 健康检测（可手工跑） | **不**写复盘报告；cron 已于 2026-07-18 删除 |
...
**② per-信号 / playbook 绩效**
- 某信号 / playbook 的 N 单胜率 / 期望，来自 `account.db.trade_experiences`（§8.5）+ playbook 统计。
- 数据源：`update_playbook_stats.py`（按引用归因更新 playbook 胜率）+ `find_similar_experience.py`（同时返回相似盈利、相似亏损与错失机会）。这些统计和案例只供 Agent 参考，不形成自动门槛。
...
> **第0步：读质量指标 JSON**
>
> 开场先读 <PROJECT_ROOT>/reports/quality/quality_metrics_YYYY-MM-DD.json（当日，由 daily_maintenance 07:55 第 7 步产出）。
> 该 JSON 重点覆盖：源达标率 / 决策卡完整率 / skip-stale 比 / action 分布 / 币种频次 / 历史经验取舍分布 / demo 可评估单 / demo-live 同向率 / 已平仓结果。
> 复盘基于该文件的数字，不从原始库临时算指标。若文件缺失或过期（mtime > 2h），在报告中标注「质量指标缺失」并继续复盘。
>
> **DXY 口径硬规则**：日报/周报/月报中的 DXY zone 只认本轮 `decision_briefing`
> 「宏观/regime」段给出的 **20 日 z-score**：`z>1.5=EXTREME` /
> `z>0.75=ELEVATED` / 其余 `NORMAL`。必须同时报告 `dxy_zone` 与 z 值；
> **禁**按 DXY 绝对点位自行判断。若 briefing
> 缺 z/zone，只能标「DXY zone 数据缺失」，不得从绝对值补判。
...
① pwsh -NoProfile -File <PROJECT_ROOT>/scripts/run_okx_python.ps1 <PROJECT_ROOT>/scripts/bookkeeping_health.py --db-root <PROJECT_ROOT>/db
   ...
   ↓
② pwsh ... <PROJECT_ROOT>/scripts/run_okx_python.ps1 <PROJECT_ROOT>/scripts/update_playbook_stats.py --apply
   # live+demo pnl 按引用归因更新 playbook 胜率；失败不阻塞日报
   ↓
③ echo '{...双段...}' | pwsh ... run_okx_python.ps1 <PROJECT_ROOT>/scripts/daily_report_writer.py --stdin --apply
   # live+demo 双段一次写；勿重复单写 demo
   ↓
④ 经 `qq_push.py` 推统一默认 target（format=3，先经 validate_push_format.py 自检，dedupe-key 标用途）
```
...
⑦ echo '{"week_start_ts":"<本周一 00:00:00>",...}' | pwsh ... daily_report_writer.py --stdin --kind weekly --apply
```

**1 号追加**：
```
⑧ echo '{"month_start_ts":"<本月 1 号 00:00:00>",...}' | pwsh ... daily_report_writer.py --stdin --kind monthly --apply
```
...
`reconcile_daily.py` 随 cron `okx-daily-maintenance` 每日 07:55 第一步执行）
...
| #9 推送 format=3 | **禁 format=4**；推前经 `validate_push_format.py` 自检 |
...
| `update_playbook_stats.py` 失败 | 写 repair_queue + 推统一 target + 继续日报（不阻塞） |
...
- **format=3**（Markdown 纯文本），经 `validate_push_format.py` 自检后 PUT。
