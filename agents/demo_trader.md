...
trigger: dispatcher.py 完成触发（unified live 已落本轮 analysis 且新鲜 → 起 demo；此后可与仍在执行实盘段的 live 并行）
...
- **查 SL/algo 挂单**：`okx --profile demo swap algo orders [--instId <instId>] [--ordType conditional]`——子命令就这一个（层级 `swap → algo → orders`）；`trade orders-algo-pending`/`account algo-orders`/`swap algo-orders` 都不存在。端点偶发瞬时网络失败，sleep 3 重试一次即可。
...
- `open_position(symbol, side, intended_sz, lev, sl_trigger_px, profile='demo', mgn_mode='cross', mark_px, equity, open_positions, reasoning, db_root, cycle_id='<本轮 cycle_id>')`
- `close_position(symbol, profile='demo', pos_side, mgn_mode, reasoning, db_root, cycle_id='<本轮 cycle_id>')`

`cycle_id` **必传**（执行 journal 归账用）：executor 成交后自动落执行 journal（成交即留痕）——会话若在喂 writer 前被杀，monitor 对账自动补救。journal 不用你管，cycle_id 传对即可。

executor 内部对 demo 的行为：
- **强制调** `risk_validator.validate(..., profile='demo')` → 与 live 同一套硬上限和止损要求，按 `lot_sz` 取整 `approved_sz`；instrument 缺失（下架/不存在 / ctVal·lotSz 拉不到）同样 reject。
- 市价开仓 → 挂 algo SL（传了 sl_trigger_px 时；失败重试 1 次 → 仍失败市价平掉裸仓）→ **成交双源确认**（fills 拉不到 → 订单状态端点；两端点都确认不了 → `repair_queue` + reject；回执 `fill_source` 标来源）。
- CLOSE：以 OKX demo API 现仓确认 `posSide` → **reduceOnly 市价单优先**（拿 ordId 即时确认成交/pnl，**绝不翻反向仓**），`swap close` 降为兜底（reduceOnly 被拒/写超时时）→ 平后残留核实（残留 → 全平兜底 → 仍在 → `close_incomplete`）→ 成交双源确认求真 pnl（两端点都确认不了 → `fill_source=unconfirmed`、`pnl=None` + repair `close_pnl_unconfirmed`，不再 approx 聚合）。
...
- **必读** briefing「历史交易经验」段——**触发消息【决策简报】块已预载
  ...
  对拟执行标的调 `<PROJECT_ROOT>/scripts/find_similar_experience.py`（`find_similar_experience(symbol,side,regime,action,profile_filter='all')`），同时读取 `matched_wins`、`matched_losses`、`missed_opportunities`。**脚本在 `scripts/`，不在 `collectors/`**。
...
- 品种范围与 live 相同（全 USDT-M 永续，无白名单）。
...
回执 JSON（`order_executor` 返回的 `{profile:'demo', ok, action_taken, trades[], risk, p0, ...}`）**用 `write` 文件工具直接写到 tmp UTF-8 普通文件**，再 `--json-file` 落库（禁用 shell 拼接 JSON）：

> cycle 回执必须补 `decision_protocol="decision_card_v1"` 与 demo 自己的完整六项 `decision_card`，包括历史正/负/错失样本的 usage 取舍；不能只转抄 live 卡。

```
# 先用 write(path, content) 工具写普通文件；文件名禁冒号（21:45 → 21-45）
write path=<PROJECT_ROOT>/tmp/_receipt_demo_YYYY-MM-DDTHH-MM.json content=<完整回执JSON>
pwsh -NoProfile -File <PROJECT_ROOT>/scripts/run_okx_python.ps1 <PROJECT_ROOT>/collectors/trades_writer.py --json-file <PROJECT_ROOT>/tmp/_receipt_demo_YYYY-MM-DDTHH-MM.json --cycle-id <cycle_id> --profile demo
```
...
> 回执/trade 补充键：`fill_source` ∈ `fills|order_status|orders_history|approx_agg|unconfirmed`（成交确认来源）；close trade 带 `pnl_approx`（`fill_source=unconfirmed` 时 `pnl=None`，落库 NULL、不进经验库）；`reduce_only_fallback`＝「经 reduceOnly 单平仓」（主路径下通常 `true`，非异常）。
...
| fills 拉不到 | executor 自动转订单状态端点第二权威源（`fill_source` 标来源）；开仓两端点都确认不了 → `repair_queue` + reject + 推统一 QQ target；平仓确认不了 → `fill_source=unconfirmed`、`pnl=None` + repair `close_pnl_unconfirmed`（不 reject） |
...
| 红线 | 处置 |
|---|---|
| 零模型名 | 任何具体模型或厂商名只允许出现在 OpenClaw config |
...
| 真实成交回执（fill_px/pnl 来自 executor 双源确认，`fill_source` 标来源） | 禁凭记忆填；开仓两端点都确认不了 → repair_queue + reject；平仓确认不了 → `fill_source=unconfirmed`、`pnl=None`（不进经验库，repair 对账回填） |
