...
本模块把它们合并成唯一的规范表与一个 ``extract_symbols`` 入口。
``news_okx_announcements.py`` 的 symbol 提取依赖公告标题结构，刻意不走本模块。

匹配规则
--------
* **英文全称**（``FULL_NAMES``）：大小写不敏感；多词名（"Bitcoin Cash"、
  "Ethereum Classic"）参与匹配且按长度倒序排在单词名之前，故 "Bitcoin Cash"
  只产出 BCH，不再顺带产出 BTC。
* **ticker**（``TICKERS``）：只匹配**大写**原文。旧实现对 ticker 也大小写不敏感，
  合并列表后 "market cap"、"near"、"lit"、"pond" 等普通英文词会全部误命中
  CAP/NEAR/LIT/POND，因此改为大小写敏感；``$btc`` 这类带 ``$`` 前缀的显式写法
  仍大小写不敏感。
...
返回值为 ``<TICKER>-USDT-SWAP`` 列表，按首次出现顺序去重。
"""
...
_TICKER_ALT = _alternation(sorted(TICKERS))
_NAME_ALT = _alternation(list(FULL_NAMES))

# 单条正则：全称（不敏感）→ $ticker（不敏感）→ 裸 ticker（大写敏感）。
# 边界：两侧不能是 ASCII 字母数字；``$`` 前也不能紧贴字母数字（"US$BTC" 不算）。
_TOKEN_RE = re.compile(
    r"(?<![A-Za-z0-9$])(?:"
    r"(?P<name>(?i:" + _NAME_ALT + r"))"
    r"|\$(?P<dollar>(?i:" + _TICKER_ALT + r"))"
    r"|(?P<ticker>" + _TICKER_ALT + r")"
    r")(?![A-Za-z0-9])"
)
...
def extract_symbols(text: str | None) -> list[str]:
    """从文本抽取 ``<TICKER>-USDT-SWAP`` 列表（按首次出现顺序去重）。"""
    return [f"{tok}-USDT-SWAP" for tok in extract_tickers(text)]


__all__ = [
    "TICKERS", "TICKER_ALIASES", "FULL_NAMES", "ZH_NAMES",
